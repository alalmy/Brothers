...
function string.starts(String, Start)
  -- print("string.starts(String, Start) is DEPRECATED use string:starts(text) installtead")
  -- uncomment if needed
...
JoBrothershanel = json:decode(url)
if not JoBrothershanel.ok or JoBrothershanel.result.status == "left" or JoBrothershanel.result.status == "kicked" then
...
redis:set(brothers..'lock:kara:'..msg.chat_id_,'on')
return "🙋🏼‍♂️*╿* أهلا عزيزي "..msg.TheRankCmd.."\n📡*╽* تم تعطيل الرفع في المجموعه \n✓"
end
function cmds_off(msg)
if not msg.Creator then return "📛*│* هذا الامر يخص {المنشئ,المطور} فقط  \n🚶"
end
redis:set(brothers..'lock:kara:'..msg.chat_id_,'off')
...
if not msg.Admin then return "♦️*│*هذا الامر يخص {الادمن,المدير,المنشئ,المطور} فقط  \n💥" end
if not MsgText[2] and msg.reply_id then
if redis:get(brothers..'lock:kara:'..msg.chat_id_) == 'off' then
...
if redis:get(brothers..'lock:kara:'..msg.chat_id_) == 'on' then
...
local Adminstall = redis:scard(brothers..'adminstall:'..msg.chat_id_)
redis:del(brothers..'adminstall:'..msg.chat_id_)
...
if not msg.Director then return "♦️*┇*هذا الامر يخص {المطور,المنشئ,المدير} فقط  \n💥" end
redis:setex(brothers..'linkGroup'..msg.sender_user_id_,300,true)
...
table.installert(All_Msgs,v.id_)
end  
end 
end 
...
local Adminstall = redis:scard(brothers..'adminstall:'..msg.chat_id_)
if Adminstall ==0 then  
return "📡*╿* اوه ☢ هنالك خطأ 🚸\n♦️╽عذرا لا يوجد ادمنيه ليتم مسحهم ✓" 
end
redis:del(brothers..'adminstall:'..msg.chat_id_)
return "🙋🏻‍♂*╿*أهلا عزيزي "..msg.TheRankCmd.."   \n♦️╽تم مسح {"..Adminstall.."} من الادمنيه في البوت \n✓"
end
...
redis:srem(brothers..'adminstall:'..msg.chat_id_,v.user_id_)
end)
end
NumMem = NumMem + 1
...
return '🧟‍♂*│*  عدد جهہآتگ آلمـضـآفهہ‏‏ ⇜ ❪ '..(redis:get(brothers..':adduser:'..msg.chat_id_..':'..msg.sender_user_id_) or 0)..' ❫ \n🐾'
end
...
if not msg.Creator then return "♦️*│*هذا الامر يخص {المطور,المنشئ} فقط  \n💥" end
return set_adminstall(msg) 
end

end -- end of installert group 
...
if not msg.SudoUser then return "♦️*│*هذا الامر يخص {المطور} فقط  \n💥" end
return '🙍🏻‍♂*│*عدد المشتركين في البوت : `'..redis:scard(brothers..'users')..'` \n📡'
end
...
if not msg.SudoBase then return "♦️*│*هذا الامر يخص {المطور الاساسي} فقط  \n💥" end
local GetVerison = https.request('https://github.com/MAXTELE/brothers.github.io/GetVersion.txt') or 0
...
if not msg.SudoBase then return"♦️*│*هذا الامر يخص {المطور الاساسي} فقط  \n💥" end
return '🎭│الاحصائيات 🏌‍♂ \n\n👥*╿*عدد المجموعات المفعله : '..redis:scard(brothers..'group:ids')..'\n🙍🏻‍♂*╽*عدد المشتركين في البوت : '..redis:scard(brothers..'users')..'\n📡'
end
---------------[End Function data] -----------------------
...
redis:set(brothers..'lave_me'..msg.chat_id_,true)  
...
function tophoto(kara,brothers)   
if brothers.content_.ID == "MessageSticker" then        
local bd = brothers.content_.sticker_.sticker_.path_          
...
text = [[🤖╿آهلا انآ بــــوت آســمـي   []]..redis:get(brothers..':NameBot:')..[[] 🗽
...
if redis:get(brothers..'linkGroup'..msg.sender_user_id_,link) then --- استقبال الرابط
redis:del(brothers..'linkGroup'..msg.sender_user_id_,link) 
redis:set(brothers..'linkGroup'..msg.chat_id_,Flter_Markdown(msg.text)) 
...
if msg.content_.ID == "MessagePhoto" and redis:get(brothers..'welcom_ph:witting'..msg.sender_user_id_) then
...
elseif msg.edited and redis:get(brothers..'lock_edit'..msg.chat_id_) then -- قفل التعديل
Del_msg(msg.chat_id_,msg.id_,function(arg,data)
...
if not redis:sismember(brothers..'kara','Start') then
redis:setex(brothers..'2DaySleep',172800,'uop90')
redis:sadd(brothers..'kara','Start')
end
if not redis:get(brothers..'2DaySleep') and redis:sismember(brothers..'kara','Start') and not redis:sismember(brothers..'kara','End') then
redis:setex(brothers..'4DaySleep',345600,'mohammad')
redis:sadd(brothers..'kara','End')
sendMsg(SUDO_ID,0,"👨‍💻↫ اهلا عزيزي المطور\n┄─┅═ـ═┅─┄\n╿ الاجباري على قناة السورس\n│مدة الاشتراك 4 ايام فقط\n╽سيتم ازاله الاشتراك تلقائيا")
end
if not redis:get(brothers..'4DaySleep') and not redis:sismember(brothers..'kara','End2') then
sendMsg(SUDO_ID,0,"🙋🏻‍♂╿هلآو حبي ❤️ \n🍁╽تم انتهاء 4 ايام الاشتراك الاجباري")
redis:sadd(brothers..'kara','End2')
end
...
local adminstall = redis:smembers(brothers..'adminstall:'..GroupS)
if #adminstall ~= 0 then
json_data =  json_data..',"Adminstall" : {'
for key,value in pairs(adminstall) do
...
local creator = redis:smembers(brothers..':KARA_BOT:'..GroupS)
if #creator ~= 0 then
...
local Save_Data = io.open("./Brothers/"..Bot_User..".json","w+")
Save_Data:write(json_data..'}}')
Save_Data:close()
sendDocument(SUDO_ID,0,"./Brothers/"..Bot_User..".json","🚸| ملف النسخه الاحتياطيه ...\n🔖| المجموعات » { "..#All_Groups_ID.." }\n📋| للبوت » "..Bot_User.."\n📆| التاريخ » "..os.date("%Y/%m/%d").."\n",dl_cb,nil)
end
