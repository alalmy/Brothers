...
json  = dofile('./Brothers/JSON.lua')
redis = redis.connect('127.0.0.1',6379)
http.TIMEOUT = 5

if not Er_cjson then
print("('\n\27[1;31m￤Pkg _ Cjson is Not installtalled.'\n\27[0m￤")
os.exit()
end
if not Er_http then
print("('\n\27[1;31m￤Pkg _ luaSec - https  is Not installtalled.'\n\27[0m￤")
os.exit()
end
if not Er_url then
print("('\n\27[1;31m￤Pkg _ Lua-cURL  is Not installtalled.'\n\27[0m￤")
os.exit()
end
if not Er_redis then
print("('\n\27[1;31m￤Pkg _ redis-lua is Not installtalled.'\n\27[0m￤")
os.exit()
end
if not Er_utf8 then
print("('\n\27[1;31m￤Pkg >> UTF_8 is Not installtalled.'\n\27[0m￤")
os.execute("sudo luarocks installtall luautf8")
os.exit()
end
...
Cr_file = io.open("./Brothers/Token.txt", "w")
Cr_file:write(Token)
Cr_file:close() 
...
function Start_Bot()
local TokenBot = io.open('./Brothers/Token.txt', "r")
...
local ok, ERROR =  pcall(function() loadfile("./Brothers/functions.lua")() end)
if not ok then 
print('\27[31m! Error File Not "BS Brothers/functions.lua" !\n\27[39m')
print(tostring(io.popen("lua Brothers/functions.lua"):read('*all')))
end

local ok, ERROR =  pcall(function() loadfile("./Brothers/locks.lua")() end)
if not ok then 
print('\27[31m! Error File Not "BS Brothers/locks.lua" !\n\27[39m')
print(tostring(io.popen("lua Brothers/locks.lua"):read('*all')))
end
...
..'¦ BS_Scrpt: \27[1;34m./Brothers/Script.lua\027[0;32m \n'
..'¦ LOGIN__IN: \27[1;34m'..login..'\027[0;32m \n'
..'¦ VERSION->: \27[1;34mv'..version..'\027[0;32m\n'
..'======================================\27[0;33m\27[0;31m'
)
local Twer = io.popen('mkdir -p pluginstall'):read("*all")
end
local ok, i =  pcall(function() ScriptFile= loadfile("./Brothers/Script.lua")() end)
if not ok then 
print('\27[31m! Error File Not "BS Brothers/Script.lua" !\n\27[39m')
print(tostring(io.popen("lua Brothers/Script.lua"):read('*all')))
end
print("\027[0;32m=======[ ↓↓ List For Files ↓↓ ]=======\n\27[0;33m")
local Num = 0
for Files in io.popen('ls pluginstall'):lines() do
if (Files:match(".lua$")) then
Num = Num + 1
local ok, i =  pcall(function() File[Files] = loadfile("pluginstall/"..Files)() end)
if not ok then
print('\27[31mError loading pluginstall '..Files..'\27[39m')
print(tostring(io.popen("lua pluginstall/"..Files):read('*all')))
else
...
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(brothers..'adminstall:'..msg.chat_id_,msg.sender_user_id_) then 
...
print("The File "..name.." Not BSing in The Source brothers")
end 

end
else
print("The File Script.lua Not BSing in The Source brothers")
...
function tdcli_update_callback(data)
	...
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/Brothers/BS.lua','./Brothers/BS.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/Brothers/locks.lua','./Brothers/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/Brothers/Script.lua','./Brothers/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/Brothers/functions.lua','./Brothers/functions.lua')
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/pluginstall/zhrfa.lua','./pluginstall/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/BrothersTeam/Brothers/master/pluginstall/games.lua','./pluginstall/games.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'100% - |█████████████|\n\n🔝*¦* السورس الى اصدار \n📟*¦* تم اعاده تشغيل السورس بنجاح')
	dofile("./Brothers/BS.lua")
	print("Update Source And Reload ~ ./Brothers/BS.lua")
	end
	...
	mmd = redis:get(brothers..":SUDO_ID:")
redis:hset(brothers..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(brothers..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(brothers..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(brothers..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(brothers..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(brothers..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(brothers..'adminstall:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(brothers..'lock_bots'..msg.chat_id_)
	ISBOT = false
...
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if IDS.Adminstall then
	for user,ID in pairs(IDS.Adminstall) do
	redis:hset(brothers..'username:'..ID,'username',user)
	redis:sadd(brothers..'adminstall:'..k,ID)
	end
	end
	if IDS.Creator then
