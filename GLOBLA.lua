...
local SUDO_USER = redis:get(GLOBLA.."Bot:KpOwnerBot")
...
local var = false
if msg.sender_user_id_ == tonumber(SUDO_USER) then
var = true
end
return var
end
...
local var = false
if user_id == tonumber(SUDO_USER) then
var = true
end
return var
end
...
local hash = "sudo:data:" .. SUDO_USER
...
if user_info_ then 
local sudoed = tonumber(SUDO_USER)
local iD_Globla = [[
...
if run == "False" or bot_id == 0 or SUDO_USER == 0 then
...
redis:set(GLOBLA.."KpOwnerBot" .. SUDO_USER, "> Bot Owner ID : " .. botowneruserid .. [[
...
getUser(SUDO_USER, outputbotowner)
--------------------function outputbot----------------------------------------
...
-----------------------------------------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' غادر$')) then
send(msg.chat_id_, msg.id_, 1, "اوك باي 😢💔💯",  1, "md")
redis:srem(GLOBLA.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
end
--------------------------------------------
...
if result.id_ then
send(msg.chat_id_, msg.id_, 1, '👤┋ العضو » ([@'..username..'])\n🔖┋ تم رفعه منشئ بالكروب \n', 1, 'md')
...
if tonumber((redis:get('GLOBLA:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
zain = '*😢┋ ليس لديك نقاط \n📬┋للحصول على نقاط ارسل الالعاب لعرض قائمة الالعاب*\n'
send(msg.chat_id_, msg.id_, 1,zain, 1, 'md')
else
...
if text == '/start' then 
if tonumber(msg.sender_user_id_) == tonumber(SUDO_USER) then
...
🙋‍♂️┋مرحبا انا بوت اسمي *(]]..(redis:get(GLOBLA..'Globlanams') or 'شارك')..[[)*
🤖┋آنا آختصاصي حمايهہ‌‏ المجموعات ،
🚸┋يمڪنني حمايه مجموعتڬ،
🌟┋ارفعني ڪادمن في المجموعهہ‌‏
...
if text:match("^المجموعات 📇$") and tonumber(msg.sender_user_id_) == tonumber(SUDO_USER) then
...
------------------------------------------------------------------------------------------------------------------------
if text:match("^المميزين العام$") then
...
------------------------------------------------------------------------------
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^منشئين$"))  then
local hash = "bot:monshis:" .. msg.chat_id_
local list = redis:smembers(GLOBLA..hash)
text = "👥┋ قائمة المشئين : \n*┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉*\n"
for k, v in pairs(list) do
...
if tonumber(result.id_) == tonumber(SUDO_USER) then
t = "مطور اساسي 👨‍🔧"
...
---------------------------------------
if idf:match("-100(%d+)") then
text = text:gsub("ايدي","[Ii]d")
if text:match("^ايدي$") or text:match("^[Ii]d$") and msg.reply_to_message_id_ == 0  then
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
...
local msgs = tonumber(redis:get(GLOBLA.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "⏱┋ تفاعلك : ( *"..ZAIN_NM(msgs).."* ) 🤹‍♂️", 1, 'md')
end
end
------------------------------------------------------------
if text == 'جهاتي' then
...
--------------------------------------------------------------
if text:match("^كشف$") or text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
...
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(SUDO_USER) then
t = "مطور الاساسـي 👨‍🔧"
...
-------------------------------------------------------------
if text == "وضع اسم البوت" or text == "تعين اسم البوت" then
if not is_devzain(msg) then
send(msg.chat_id_, msg.id_, 1, '👨‍🔧┋ للمطور الاساسي فقــــــــط', 1, 'md')
else
redis:setex(GLOBLA..'botts:namess'..msg.sender_user_id_,698,true)
...
local users = redis:scard(GLOBLA.."bot:userss")
local botnamess = redis:get(GLOBLA.."Globlanams") or "شارك"
...
send(msg.chat_id_, msg.id_, 1, "📌┋ رتبتك بالبوت » (*"..t.."*)\n⚔┋ رتبتك بالكروب » " ..rtpa.. " \n‏" , 1, "md")
end,nil)
end
...
send(msg.chat_id_, msg.id_, 1, "🔖┋البوتات بالطرد مقفوله بالتأكيد \n👤┋ بواسطه ← "..tmGlobla(msg).."\n ‏ ", 1, "md")
end
...
if lockGlobla[2] == "الدخول" then
if not redis:get(GLOBLA.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🔖┋ تم قفل الدخول \n👤┋ بواسطه ← "..tmGlobla(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "🔖┋ الدخول مقفوله بالتأكيد \n👤┋ بواسطه ← "..tmGlobla(msg).."\n ‏ ", 1, "md")
end
...
send(msg.chat_id_, msg.id_, 1, "🔖┋ البوتات بالطرد مفتوحه بالتأكيد \n👤┋ بواسطه ← "..tmGlobla(msg).."\n ‏ ", 1, "md")
end
...
send(msg.chat_id_, msg.id_, 1, "📉┋عدد منشئين ← *"..kpmonshis.."*\n🔖┋ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
...
os.execute('rm -rf GLOBLA.lua') 
os.execute('wget https://raw.githubusercontent.com/alalmy/GLOBLA/master/GLOBLA.lua') 
send(msg.chat_id_, msg.id_, 1, "👤┋ بواسطه » "..tmGlobla(msg).."\n🔖┋ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('GLOBLA.lua')  
...
---------------------------------------------------------------------
if is_sudo(msg) and (text:match("^تيست$")) then
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
load_config()
setnumbergp()
send(msg.chat_id_, msg.id_, 1, "البوت شـغــال بالتأكيد🚀 ", 1, "md")
end end
-----------------------------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, '©️┋معرفك ↬ ('..(result.username_..')\n👤┋الاسم ↬ ('..result.first_name_ or "GLOBLA")..')', 1, 'md')
end
...
-----------------------------------------------------------------------------------------
if text:match("^موقعي$") or text:match("^معلوماتي$") then
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
tdcli_function ({ID = "GetChatMember",chat_id_ = msg.chat_id_,user_id_ = result.id_},function(arg,da) 
if da.status_.ID == "ChatMemberStatusCreator" then
...
-------------------------------------
if text == ''..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' هينه' or text == ''..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' رزله' then
...
return false  end
local GLOBLA = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐","ها ابن الحنينه، ليش متسكت وتنجب 🌚","تعال لك كواد فرخ دودكي مستنقع 😹👻","حبيبي شدا تحس انته هسه من تكمز !؟ دبطل حركاتكم هاي 🌝❤️","يمعود هاذا من جماعة لا تعورني 😹"}
...
-------------------------------------
if text == ''..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' بوسه' or text == ''..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(GLOBLA_SUDO) then
...
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
local GLOBLA = {'اسمي '..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' 😌👌','تره اسمي '..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' يالجريذي ☹️💥'  }
send(msg.chat_id_, msg.id_, 1,''..GLOBLA[math.random(#GLOBLA)]..'', 1, 'md')
end
end
-------------------------------------
if text == ''..(redis:get(GLOBLA..'Globlanams') or 'شارك')..'' then
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
local GLOBLA = {'اي نعم اني '..(redis:get(GLOBLA..'Globlanams') or 'شارك')..' 😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..GLOBLA[math.random(#GLOBLA)]..'', 1, 'md')
end
end
-------------------------------------
...
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then local GLOBLA = {"مووووووووواححح💋😘","مابوس ولي😌😹","خدك/ج نضيف 😂","البوسه بالف حمبي 🌝💋","خلي يزحفلي وابوسه 🙊😻","كل شويه ابوسه كافي 😏","ماابوسه والله هذا زاحف🦎","محح هاي لحاته صاكه💋"}
send(msg.chat_id_, msg.id_, 1,''..GLOBLA[math.random(#GLOBLA)]..'', 1, 'md')
end
end
-------------------------------------
...
if not redis:get(GLOBLA..'lock:add'..msg.chat_id_) then
local GLOBLA = {"نجب انت لا الطمك ع حلكك 😤","بيك مرض الا تتحارش😒","اهو اجو الملطلطين ويغلطون" }
send(msg.chat_id_, msg.id_, 1,''..GLOBLA[math.random(#GLOBLA)]..'', 1, 'md')
end
end
-------------------------------------
